...
export const createProfessional = async (req: AuthRequest, res: Response) => {
  try {
    const { name, email, phone, specialties, working_hours } = req.body;
    ...
    if (existingProfessional) {
      return res.status(400).json({ error: 'Já existe um profissional com este email' });
    }
...
export const updateProfessional = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { name, email, phone, specialties, working_hours, is_active } = req.body;
